...
void Nodes::finish(){
    if(type == 3){
        ...
        for (int i = 0; i < arrayLength; i++)
            preArr[i] = 0;
        fileInput.open(("/Users/macbookpro/Desktop/TestThroughPut/Writeout3.txt"), ios::in);
        if(fileInput){
            int i = 0;
        ...
        fileInput.close();
        fileOutput.open(("/Users/macbookpro/Desktop/TestThroughPut/Writeout3.txt"), ios::out);
        for (int i = 0; i < arrayLength; i++) {
            fileOutput << receivedMsgCount[i] + preArr[i] << " ";
//            fileOutput << "Without '+' :"<< receivedMsgCount[i] << " " << endl;
        }
        fileOutput << endl;
        fileOutput.close();
...
//        printf("Interval Count: %i\n", intervalCount);
    ...
    if (getIndex() == 35) {
//        fileOutput.open("/Users/macbookpro/Desktop/TestThroughPut/statistical.txt");
//        char c =
        cout << "All received packets: " << sum1 << endl;
        cout << "Throughput: " << (double) (100 * sum1) / (1 * 1000 * 1000 * received_node * TIMEOUT) << endl;
    }
}
